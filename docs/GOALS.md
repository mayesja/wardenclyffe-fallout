...
* **Coding Proficiency:** The creator is completely focused on gameplay design and has minimal development, programming, or terminal scripting experience. The creator relies on Gemini to write, structure, and maintain 100% of the project's source code.
* **Hardware Setup:** All development operations are run locally on a Chromebook.
* **Tooling Framework:**
  * **Crostini Linux Terminal:** Used exclusively for executing straightforward, copy-pasted operational commands (such as running local development servers).
...
1. **Gemini has a finite memory:** Gemini will lose the thread after an extended session or 30-40 prompts.
   * Key signs that Gemini has lost the thread and needs a fresh sandbox thread:
     1. **Inaccurate Line Tracking:** References line numbers that do not match the editor (off by 5-10 lines or in a completely different section).
     2. **Comment Disconnects:** Search targets or comments suggested by Gemini do not match the actual code file.
     3. **Variable/Function Hallucinations:** Directs edits toward non-existent functions or variables, apologizing repeatedly without refreshing its context.
2. **Gemini is great until it's not:** Code updates and verbose commenting are highly effective until memory saturation occurs.
3. **Proposed Workflow (Class 2 Sandbox Pattern):**
   1. Upload `.md` and source code files; explain the next development task.
   2. Have Gemini generate a focused prompt describing the task.
   3. Begin a second/separate Class 2 sandbox thread.
   4. Upload source files and start the task using the generated prompt.
   5. Test functionality locally. Upon completion, request a wrap-up summary for `DEVELOPMENT_MANUAL.md`.
   6. Close the sandbox, update docs, and use the main thread for GitHub pushes.
4. **Self-Recognition & Error Handling:** When Gemini makes mistakes, it tends to apologize and push forward anyway. Forcing a hard execution halt and opening a fresh sandbox thread is the safest recovery path.
5. **Insights into Goal #3 vs. Goal #4:** Learning Gemini's operational limits directly fulfills Goal #3, even if memory hurdles temporarily slow down Goal #4 (building an enjoyable game).
...
7. **The Encapsulated Engine Refactor Verification:** Successfully executed a full 3-step state encapsulation refactor (`gameState` object tree) using clean Class 2 sandbox threads. This confirmed that isolating single tasks in stateless execution windows completely prevents context drift and loose-global pollution.
